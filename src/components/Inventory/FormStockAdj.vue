    ...
    <form class="flat-card">
      <div class="card-content">
        ...
        <b-field grouped group-multiline>
          ...
          <b-field label="Branch/Outlet" expanded>
            <b-select expanded placeholder="Select One" v-model="cbranch">
              <option
...
export default {
  ...
  methods: {
    ...
    editStock(id) {
      this.isLoading = true;
      this.axios.get(`/stock_adjustment/get/${id}`).then(res => {
    ...
    submitStock() {
      ...
      let bodyRequest = {
        ...
        tnote: this.tnote,
        tstaff: this.tstaff,
        cbranch: this.cbranch
      };

      if (this.id) {
        submit = this.axios.post(
          `/stock_adjustment/update/${this.id}`,
      ...
      submit
        .then(res => {
          ...
          if (Number.isNaN(err)) {
            ...
            if (!this.id) {
              // this.id = err;
              // this.$router.push("/inventory/stock_adjustment/edit/" + err);
            } else {
              this.goBack()
            }
    ...
    postItem() {
      ...
      this.axios
        .post(`/stock_adjustment/add_item/${this.id}`, urlEncoded(dataAccount))
        .then(res => {
    ...
    deleteAccount(id) {
      this.axios
        .get(`/stock_adjustment/delete_item/${id}`)
        .then(res => {
          this.editStock(this.id);
        })
        .catch(err => {
