            ...
            <div class="column is-12">
              ...
              >
                ...
                <template slot-scope="props">
                  <b-table-column field="code" label="Code">CIN-00{{ props.row.no }}</b-table-column>
                  <b-table-column field="currency" label="Currency">{{ props.row.currency }}</b-table-column>
                  <b-table-column field="date" label="Date">{{ props.row.dates }}</b-table-column>
                  <b-table-column field="acc" label="Account">{{ props.row.account }}</b-table-column>
                  <b-table-column field="notes" label="Notes">{{props.row.notes}}</b-table-column>
                  <b-table-column field="customer" label="Customer">{{ props.row.customer }}</b-table-column>
                    ...
                    <div class="buttons has-addons">
                      ...
                      >
                        <span
                          ...
                          @click.prevent="editCashin(props.row.id)"
                        >
                      ...
                      >
                        <span
                          ...
                          @click.prevent="deleteCashin(props.row.id)"
                        >
  ...
  methods: {
    ...
    loadAsyncCashinList() {
      ...
      if (this.search) {
        ...
        ajax = this.axios.post("/cashin/search", bodyRequest);
      } else {
        ajax = this.axios.post("/cashin", bodyRequest);
      }
      ajax
        .then(res => {
          let cashin = res.data.content.result
this.count = res.data.content.record
          console.log(cashin);
          if (cashin) this.$store.commit("cashin/addCashinList", cashin);
          else this.$store.commit("cashin/addCashinList", []);
          this.loading = false;
        })
        .catch(error => {
          console.log(error);
    ...
    updateDate(value) {
      this.$store.commit("cashin/updateDate", value);
    },
    // When Filter Button Clicked
    loadFilter() {
      this.$store.commit("cashin/filter");
      this.loadAsyncCashinList();
    },
    isFilterShow() {
      this.$store.commit("cashin/isFilterShow", !this.showFilter);
    },
    // Change Status
    changeStatus(id) {
      this.axios.get(`/cashin/confirmation/${id}`).then(res => {
        this.loadAsyncCashinList();
      });
    },
    // Show Invoice
    showInvoice(id) {
      this.axios.get(`/cashin/invoice/${id}`).then(res => {
    ...
    deleteCashin(id) {
      swal({
        title: "Are you sure?",
        text: "Once deleted, you will not be able to recover this cashin data!",
        icon: "warning",
        buttons: true,
        dangerMode: true
      }).then(willDelete => {
        if (willDelete) {
          this.axios
            .get(`/cashin/delete/${id}`)
            .then(res => {
              swal("Poof! Your cashin data has been deleted!", {
                icon: "success"
              });
              this.loadAsyncCashinList();
            })
          ...
          swal("Your cashin data is safe!");
        }
      });
    },
    // Delete Checked Cashin
    deleteCheked() {
      swal({
        title: "Are you sure?",
        text: "Once deleted, you will not be able to recover this cashin data!",
        ...
        if (willDelete) {
          ...
          this.axios
            .post("/cashin/delete_all", urlEncoded(bodyRequest), {
              headers: {
                "Content-Type": "application/x-www-form-urlencoded"
              }
            })
            .then(res => {
              swal("Poof! Cashin data has been deleted!", {
                icon: "success"
              });
              this.checkedRows = [];
              this.loadAsyncCashinList();
    ...
    editCashin(id) {
      this.$router.push("/transaction/cashin/edit/" + id);
    },

    reset(event) {
      this.$store.commit("cashin/reset");
    },
    next() {
      this.$store.commit("cashin/incrementPage");
      this.loadAsyncCashinList();
    },
    // Next
    previous() {
      if (this.page > 1) {
        this.$store.commit("cashin/decrementPage");
        this.loadAsyncCashinList();
      }
    },
    searchCashin: debounce(function(event) {
      this.$store.commit("cashin/updateSearch", event);
      this.$store.commit("cashin/reset");
      this.$store.commit("cashin/filter");
      this.loadAsyncCashinList();
    }, 500),
...
created() {
    if (!this.cashinList.length) this.loadAsyncCashinList();
  },
  computed: {
    ...mapState({
      page: state => state.cashin.page,
      offset: state => state.cashin.offset,
      cashinList: state => state.cashin.cashinList,
      limit: state => state.cashin.filter.limit,
      showFilter: state => state.cashin.filter.showFilter,
      filterDate: state => state.cashin.filter.date,
      search: state => state.cashin.filter.search
    })
  }
};
</script>
