<template>
  <div>
    ...
    <div class="flat-card">
      <div class="card-content">
        <!-- <h4 class="title is-5">Unit - Global Permata</h4> -->
            ...
            <form @submit.prevent="submitUnit">
              <div class="modal-card" style="width: auto">
                <header class="modal-card-head">
                  <p class="modal-card-title">{{id?"Edit": "Add New"}} Unit</p>
                </header>
                ...
                <footer class="modal-card-foot">
                  <button class="button is-primary" @click.prevent="submitUnit()">Save</button>
                  <button
                ...
                <b-table-column label="Action">
                  <div class="buttons has-addons has-text-centered">
                    <b-tooltip
                      ...
                      animated
                    >
                      <span class="button is-small is-info" @click.prevent="editUnit(props.row.id)">
                    ...
                    >
                      <span
                        ...
                        @click.prevent="deleteUnit(props.row.id)"
                      >
  ...
  methods: {
    ...
    loadAsyncUnitList() {
      ...
      if (this.search) {
        ...
        ajax = this.axios.post("/unit/search", bodyRequest);
      } else {
        ajax = this.axios.post("/unit", bodyRequest);
      }
      ajax
        .then(res => {
          let unit = res.data.content.result
this.count = res.data.content.record
          if (unit) this.$store.commit("unit/addUnitList", unit);
          else this.$store.commit("unit/addUnitList", []);
          this.loading = false;
        })
        .catch(error => {
          console.log(error);
    ...
    loadFilter() {
      this.$store.commit("unit/filter");
      this.loadAsyncUnitList();
    },
    isFilterShow() {
      this.$store.commit("unit/isFilterShow", !this.showFilter);
    },
    // Change Status
    changeStatus(id) {
      this.loading = true;
      this.axios.get(`/unit/publish/${id}`).then(res => {
        this.loadAsyncUnitList();
      });
    },
    // Delete Unit
    deleteUnit(id) {
      swal({
        title: "Are you sure?",
        text: "Once deleted, you will not be able to recover this unit data!",
        icon: "warning",
        buttons: true,
        dangerMode: true
      }).then(willDelete => {
        if (willDelete) {
          this.axios
            .get(`/unit/delete/${id}`)
            .then(res => {
              swal("Poof! Your unit data has been deleted!", {
                icon: "success"
              });
              this.loadAsyncUnitList();
            })
    ...
    deleteCheked() {
      swal({
        ...
        if (willDelete) {
          ...
          this.axios
            .post("/unit/delete_all", urlEncoded(bodyRequest), {
              headers: {
                "Content-Type": "application/x-www-form-urlencoded"
              }
            })
            .then(res => {
              swal("Poof! Unit data has been deleted!", {
                icon: "success"
              });
              this.checkedRows = [];
              this.loadAsyncUnitList();
    ...
    editUnit(id) {
      this.axios.get(`/unit/get/${id}`).then(res => {
    ...
    submitUnit() {
      ...
      if (this.id) {
        ...
        submit = this.axios.post("/unit/add", urlEncoded(unitData), {
          headers: {
            "Content-Type": "application/x-www-form-urlencoded"
          }
        });
      ...
      submit
        .then(res => {
          ...
          swal("Success!", "Unit submit successfully!", "success");
    ...
    next() {
      this.$store.commit("unit/incrementPage");
      this.loadAsyncUnitList();
    },
    // Next
    previous() {
      if (this.page > 1) {
        this.$store.commit("unit/decrementPage");
        this.loadAsyncUnitList();
      }
    },
    searchUnit: debounce(function(event) {
      this.$store.commit("unit/updateSearch", event);
      this.$store.commit("unit/reset");
      this.$store.commit("unit/filter");
      this.loadAsyncUnitList();
    }, 500)
...
created() {
    if (!this.unitList.length) this.loadAsyncUnitList();
  },
  computed: {
    ...mapState({
      page: state => state.unit.page,
      offset: state => state.unit.offset,
      unitList: state => state.unit.unitList,
      limit: state => state.unit.filter.limit,
      showFilter: state => state.unit.filter.showFilter,
      search: state => state.unit.filter.search
    })
  }
};
</script>
